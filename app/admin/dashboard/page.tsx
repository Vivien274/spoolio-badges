...
async function getAllFiches(): Promise<Fiche[]> {
  ...
  const { data } = await supabase
    .from('fiches')
    .select('id, token, nfc_encoded_at, type, is_claimed, data, failed_attempts, locked_until, created_at, updated_at')
...
export default async function DashboardPage() {
  ...
  const activated = fiches.filter((f) => f.is_claimed).length
  const pending = fiches.filter((f) => !f.is_claimed).length
  const encoded = fiches.filter((f) => !!f.nfc_encoded_at).length
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-6xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
            <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">NFC encodé</p>
            <p className="text-4xl font-black text-emerald-500">{encoded}</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          <p className="text-sm text-gray-500 mb-4">Chaque fiche est créée avec un token unique. L&apos;activation se fera au premier scan avec choix du mot de passe.</p>
          <GenerateForm />
        </div>
